...
library(lme4)
lmer.1 <- lmer(data.df.avg_ch4_smilk ~ . - id + 
                 (1 | id), data=data.plus.df)
...
for (b in 1:B)
{
  ...
  for (k in 1:K)
  {
    ...
    data.train.df = data.frame(data.new.df[itrain,], pc.mir$x[,1:60])
    ...
    data.test.df = data.frame(data.new.df[-itrain,], pc.test[,1:60])
